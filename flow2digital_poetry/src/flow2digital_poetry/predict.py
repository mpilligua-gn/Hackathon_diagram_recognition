...
class predict_mode:
    ...
    def predict_flowchart(self, img_path, arrow_metadata=None, save_path=None):
        # eval model result
        # print('-----eval begin-----')
        cfg = get_cfg()
        ...
        cfg.MODEL.WEIGHTS = os.path.join("./ckpt/model_final_80k.pth")  # "model_final_keypoint_detection.pth")  # path to the model we just trained
        ...
        # predictor.model.roi_heads.box_predictor.cls_score = nn.Linear(in_features=1024, out_features=8, bias=True).cuda()
    ...
    def shapes_connect(self, keypoint, cls, box):
        ...
        if cls == 0:  # circle
            ...
            diameter = (box[3] + box[1] - box[0] - box[2]) / 2
            ratio *= diameter
            top_left = [box[0] + ratio, box[3] + ratio]
            lower_left = [box[0] + ratio, box[1] - ratio]
            top_right = [box[2] - ratio, box[3] + ratio]
            lower_right = [box[2] - ratio, box[1] - ratio]
            center = [(box[0] + box[2]) / 2, (box[3] + box[1]) / 2]
        ...
        if cls == 2:  # long oval
            ...
            lower_right = [box[2], box[1]]
            points = torch.tensor([left, right, down, up, top_left,
                                  top_right, lower_left, lower_right])
        if cls == 3:  # hexagon
            up1 = [box[0] + (box[2] - box[0]) / 3, box[3]]
            up2 = [box[2] - (box[2] - box[0]) / 3, box[3]]
            down1 = [box[0] + (box[2] - box[0]) / 3, box[1]]
            down2 = [box[2] - (box[2] - box[0]) / 3, box[1]]
            down = [(box[0] + box[2]) / 2, box[1]]
            left = [box[0], (box[3] + box[1]) / 2]
            right = [box[2], (box[3] + box[1]) / 2]
            center = [(box[0] + box[2]) / 2, (box[3] + box[1]) / 2]
        ...
        if cls == 4:  # parallelogram
            up = [(box[0] + box[2]) / 2, box[3]]
            down = [(box[0] + box[2]) / 2, box[1]]
            left = [box[0] + (box[2] - box[0]) / 10, (box[3] + box[1]) / 2]
            right = [box[2] - (box[2] - box[0]) / 10, (box[3] + box[1]) / 2]
